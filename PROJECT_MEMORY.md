...
- **Current phase:** Phase 1 数据采集 **9/9 完成**(2026-07-03);**Module A 收官 6/6**(A1 ✅ 07-04 · A2 ✅ 07-05 · A4 ✅ 07-07 · A3a ✅ 07-09 · A5 ✅ 07-10 · **A6 ✅ 2026-07-11**,A3b 为 8/1 补充项)。milestone 达成:master.gpkg(843,062 行,私仓)+ 覆盖率四个数(栋数/GFA × 全市/EULUC-scoped 双权重双口径)已报,**Module B 全量 ML 判 GO**(规则法天花板实证:同分母 52.2% vs EULUC-only 54.4%)。**Module B 进行中**:B1 基线版 ✅ 2026-07-12(收账见 §2),B0 抽检 10 栋 + B2 补标 119 栋两份清单待 owner 人工;retail 特征版随 A3b 8/1。**Module C 进行中**:EnergyPlus 26.1.0 装毕(NREL 源);office archetype shoebox 按 GB 50189-2015 建成;52 格仿真矩阵 v0 零失败(healthcare EUI 最高 / residential 最低,量级合理);两个 GB 50189 窗参数未对标准原文核实(见 §3)。**2026-07-24 上游巡检**:发现 buildings.city **V2.0**(2026-05-16 发布,MIT)已用纯 Python 管线取代论文 v1.0 的 Rhino/Grasshopper 链路,B3 实现方式 / Module C 后半段路线 / Phase 7 对齐目标三处受影响,详见 §2 2026-07-24 行。**三道决策闸门**:① 7 月底 —— 任务 4 验证实验过不过,决定 14 类参数表走切换还是自研;② 8 月 1 日 —— EULUC 盲区补扫做不做,判据以 B1 临港 `euluc_out=100%` 实证为准,不必等 B3;③ 8 月上旬 —— B4 结果决定最终模型用不用零售特征。
...
|---|---|---|---|
| 2026-07-24 | **上游巡检定案 + 三闸门设立**:实查 `City-Syntax/buildings.city` V2.0(2026-05-16,MIT,commit `fde303a`)与 `buildings.sg`(`b95ccce`)—— 论文描述的 v1.0 Rhino+Grasshopper 链路已被纯 Python 取代,V2.0 含 ml-service(~700 行 FastAPI,RF 超参与论文一致,特征纯几何 5 个)+ simulation-service(~4,700 行,`templates.json`→epinterface→IDF→eppy 批跑),`buildings.sg` 带 **31 字段 GeoJSON schema**(118,782 栋)= buildings.shanghai 对齐目标。四模块处置:**A 分类不动**(上海无楼名词典、无政府逐栋标签,复杂是数据环境所迫,四法证是亮点);**B 设计不变**,B3 改造其 `random_forest_model.py`(剥 FastAPI 壳 + 加 EULUC 先验/ep 特征 + 换 in-parcel 评估口径);**C 设闸门** —— 验证实验(任务 4)全绿才切 `templates.json` + builder,任一红灯保留自研管线只借参数表格式,52 格留档作交叉验证;**E 校准不动**(逐月 144 点牌更好)。Phase 7 路线确认 fork + 换数据 + 换 config,新增已知工程量:84.3 万栋 GeoJSON 预计 ~1GB(新加坡 11.9 万栋 = 135MB),浏览器直载不可行,**必须 Tippecanoe 切 MBTiles**。**论文原贡献 3(首个纯 Python UBEM 管线)被 V2.0 证伪,须改写**(方向待拍板,见 §6) | 上游是活的,论文是死的 —— 5 月立项以论文 PDF 为唯一权威,漏了 Release/CHANGELOG。**沉淀新规矩**:凡框架级复刻项目,每逢新模块开工前先巡检上游 Release/CHANGELOG(十分钟),本项目下一检查点 = Module D 开工前。切换本身有学习与调试成本,净节省须待任务 4 验证后才可诚实估计 | claude.ai 项目文件 `UBEM_20260724_yuqian_review_taskplan.md`(六批二十项任务表 + 三闸门 + 自省);本行。**引用义务**:Lyu, Wong, Soh, Wang & Ang (2026) *CEUS* 128:102453;软件版本须写清「论文描述 v1.0 / 本项目实用 V2.0」;fork 时 README 声明上游与改动范围(MIT) |
| 2026-07-12 | **B1 基线版收账:零 POI(形态+邻域+EULUC+ep)RF 与 EULUC-only 天花板持平** —— in-parcel 136 同分母 54.4% [46.3,62.5] vs 全规则 52.2 / EULUC-only 54.4(CI 重叠);全匹配 156 口径 52.6%、训练真值 153 OOF 53.6%;GFA 加权 65.7%(大体量更准);逐类 residential F1 0.80 / office 0.50 / education 0.25 / mall 0.24,<5 样本五类(gov/healthcare/culture/other_public/unclear)全 0 标"不可学,待 B2";特征重要性前三全为邻域系;citywide 诊断 mall 27.6% / unclear 11.7% 系 balanced 小类外溢(仅诊断不落 master);**临港窗口 euluc_out=100%** 盲区实证(喂 8/1 拍板),张江 euluc_out 仅 1.5% | 摸底不是考试,如实收账;V009/V096/V141 剔出训练真值;5km GroupKFold 空间 CV(6 组)防泄漏;B2 清单 119 栋 = 改判 3 + hotel 16 + sport_culture 18(池实测 6,107)+ 楼层 6 箱 42 + 低置信 40(临港/张江各 10),seed=20260712 | 主仓 PR #12(squash `e2ee559`:features.py + b1_baseline.py + config/b1_baseline.yaml + 单测)+ main 直入 `data/reference/b1_*.csv` ×6 + `data/raw/validation/b2_annotation_list_v0.csv`;B0 抽检另行 `6347ecc` |
...
- [@claude-code, next] **任务 4 —— V2.0 管线验证实验**(闸门 1,7 月底截止):Codespace 克隆 `City-Syntax/buildings.city`,六项检查 ① `epinterface` 能否干净安装 ② 仓库内 IDF 目标 E+ 24.2 与本地 26.1 的版本差处理 ③ template_builder 的 ideal-loads-only HVAC 与 52 格矩阵建模方式一致性 ④ 天气换上海 CSWD ⑤ office 单类生成并跑通 ⑥ 结果与 52 格 office 对比量级。**全绿才切换;任一红灯保留自研管线、只借参数表格式。** 与 B0/B2 人工活互不等待。
- [@claude-code, next] **任务 5 —— B3 代码备锅**:改造上游 `random_forest_model.py` 为独立脚本(5 几何特征 + EULUC 先验 + ep 特征;训练接口预留「156+119 剔弱标签」;in-parcel 评估口径),与任务 4 并行,互不等待。
- [~~done 2026-07~~] **Module C 起步**:EnergyPlus 26.1.0 装毕(NREL 源);office archetype shoebox 按 GB 50189-2015 建成;52 格仿真矩阵 v0 零失败(healthcare 最高 / residential 最低,量级合理)。**留档作闸门 1 的交叉验证基准。**
- [@owner] **GB 50189-2015 两个窗参数核实**:对标准原文查两个悬置值(52 格矩阵建模时暂用估值),随参数表格式切换一并清账;约半小时。
- [@owner, next] **B2 补标 119 栋 + B0 抽检 10 栋人工**:`data/raw/validation/b2_annotation_list_v0.csv`(分层构成见 §2 2026-07-12 行;reassess V009/V096/V141 置顶,actual_class/actual_floors/notes 待填)+ `data/raw/validation/b0_spotcheck_10.csv`(位置/层数/标签三查);回传后 @claude-code 跑 B1 重训评估 + sport_culture 拆分评估。
- [~~done 2026-07-12~~] Module B 基线版(B1):收账见 §2 2026-07-12 行(PR #12);验收余项 sport_culture 拆分 / culture·hotel 补样评估移交 B2 清单,retail 特征版对比随 A3b 8/1。
- [~~done 2026-07-12(采集段)~~] **CNBH-10m 采集入库**:Zenodo DOI 10.5281/zenodo.7923866(latest,CC-BY-4.0)X121Y31+X123Y31 两瓦片 MD5 命中 Zenodo → GADM 4.1 市界切裁 merge 单文件,zip 76.3MB(<100MB 未分片)入私仓(PR #2 `82aaddb`),双 MD5 记暗号本;脚本+元数据主仓 PR #13。⚠️ 像元 max 41.7m,超高层严重饱和(上海中心 632m 不可见)。**余留分析段(Module C 前)**:误差分布 vs GB 正演 + 28 栋未配 supertall 裁定 —— 归下一对话。
...
- **新增已知工程量(2026-07-24)**:导出 31 字段 GeoJSON 预计 ~1GB,浏览器直载不可行,必须 Tippecanoe 切 MBTiles(`buildings.sg` 同款方案,论文 3.4.1 明写);31 字段 schema 取自 `sg_buildings_v5.geojson` 单条记录,执行时须全表扫一遍验字段一致性(GeoJSON 各 feature 字段可异构)。
...
| CNBH-10m / 3D-GloBFP height raster | **ingested 2026-07-12 via 云端 git 通道**(Zenodo DOI 10.5281/zenodo.7923866 latest / concept 10.5281/zenodo.7015081,CC-BY-4.0;源瓦片 X121Y31 `72fccc03…` + X123Y31 `cbf6c31…` 均命中 Zenodo 元数据;GADM 4.1 市界切裁 merge 18837×14622 px float32 EPSG:32651 原生 10m;zip **76.3MB** MD5 `075f9259…`、内层 tif `e15fc47b…` 记暗号本;界内有高度像元 16,644,950,高度 p50 16.7 / max **41.7m** ⚠️ 超高层饱和,误差分析归下一对话;3D-GloBFP 备选未动用) | 主仓 `data/raw/cnbh/README.md` + `scripts/cnbh_clip_shanghai.py`;私仓 `cnbh10m_shanghai_2020.tif.zip` | 2026-07-12 |
...
- [@owner, surfaced 2026-07-24] **论文贡献 3 改写方向待拍板**:原表述「首个纯 Python UBEM 管线(eppy/geomeppy/honeybee)取代 Rhino/Grasshopper」被 V2.0(2026-05-16)证伪 —— 无论切不切换到其代码,其发布在先(虽未经同行评审),该主张在我们投稿时已不成立。**甲**(Claude 倾向)= 改写为「首次将 Buildings.city 框架独立迁移到数据环境迥异(无楼名词典、无政府逐栋标签、夏热冬冷)的中国超大城市,并系统扩展:月度校准、GB 参数体系、综合体方法学、法证级数据 QA」,即原文自我标榜的 transferability 的**第一个独立外部验证**;**乙** = 保留自研管线叙事,贡献 3 改写为「面向中国数据环境的全流程适配」,如实引用 V2.0 存在。贡献 1(逐月校准)/2(夏热冬冷超大城市全覆盖开源 UBEM)/4(综合体)不受影响且分量相对上升。**动笔 Methods 前必须定,不阻塞任务 4/5**;建议随闸门 1 结果一并拍板。
- [@owner, surfaced 2026-07-24] **半监督备用牌是否预先批准**:若 275 栋(156+119)训练精度不足,备用牌 = 照上游楼名词典法用 POI 名字批量生成机器标签作训练辅助(训练可吃噪声标签,**评估仍只认人工真值**,半监督标准做法)。此举把 2026-07-11 弱标签铁律的「只作特征」扩展为「可作带噪训练标签」,**属铁律适用范围的实质修订,须 owner 明文批准,不得默改**。默认不启用,倾向等 B3 结果出来再议。
...
**账本已对齐至 2026-07-27。** 三件人工待办:① **B0 抽检 10 栋三查**(B3 前提,半小时,**建议先于 B2 —— 先验货再下料**);② **B2 补标 119 栋**(V009/V096/V141 改判置顶);③ GB 50189 两个窗参数核实。**机器侧可即刻并行、不等人工**:任务 4 V2.0 验证实验(闸门 1,7 月底)+ 任务 5 B3 代码备锅。**8/1 双动作**:A3b retail 抓取(先 10 次试针)+ 闸门 2 拍板(判据用 B1 临港 `euluc_out=100%` 实证,不等 B3)。CNBH-10m 采集段已完成(§3/§4 2026-07-12),余误差分析 + 28 栋未配 supertall 裁定(Module C 前)。悬案余:ep 语义(供应商)、415/6,027 身份、超高层名单细则、**贡献 3 改写方向**、**半监督备用牌批准**、mall 召回不可测(A3b 销)。

...
**等 owner 三件人工 + 8/1 双拍板**——①B0 抽检 10 栋三查;②B2 补标 119 栋标注(回传后 B1 重训 + sport_culture 拆分评估);③8/1 A3b retail 特征版对比 + EULUC 盲区补扫二选一(B1 已备实证:临港窗口 euluc_out=100%)。CNBH-10m 采集(§3,Module C 前)可独立先行。悬案余:ep 语义(供应商)、415/6,027 身份、超高层名单细则 + 28 栋未配 supertall(随 CNBH)、V009/V096/V141 改判(B2 置顶)、mall 召回不可测(A3b 销)。
